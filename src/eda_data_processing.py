...
def date_selection_ui(df, file_key):
    ...
    def parse_dates_series(s):
        ...
        p1 = pd.to_datetime(s, errors="coerce", infer_datetime_format=True, dayfirst=True)
        p2 = pd.to_datetime(s, errors="coerce", infer_datetime_format=True, dayfirst=False)
    ...
    if not date_columns:
        ...
        if candidate_scores:
            date_columns = sorted(candidate_scores, key=candidate_scores.get, reverse=True)
    ...
    if date_col != "None":
        ...
        if not is_datetime64_any_dtype(df[date_col]):
            ...
            if success >= 0.80:
                st.success(f"Conversion successful for '{date_col}' (YYYY-MM-DD). Parsed: {success:.0%} of values.")
            else:
                st.warning(f"⚠️ Please select correct date column. Conversion for '{date_col}' parsed only {success:.0%}.")
        else:
            st.info(f"'{date_col}' is already a datetime column. Normalized for preview.")
...
def channel_selection_ui(df, file_key):
    ...
    channel_like = next((col for col in eligible_columns if str(col).strip().lower() == "channel"), None)
    ...
    if channel_available == "Yes":
        ...
        with cols[0]:
            ...
            channel_col = st.selectbox("Select channel column", options, key=f"channel_col_{file_key}")
        with cols[1]:
...
def _get_rename_store(scope: str):
    return st.session_state.rename_entries_melted if scope == 'melted' else st.session_state.rename_entries_df
...
def apply_renames(df: pd.DataFrame, file_key: str, scope: str = 'df'):
    ...
    rename_map = {e.get("column", ""): e.get("new_name", "")
                  for e in entries if e.get("column") and e.get("new_name")}
    if rename_map:
...
def rename_other_columns_ui(df: pd.DataFrame, file_key: str, scope: str = 'df'):
    ...
    for i, entry in enumerate(store[file_key]):
        ...
        with cols[0]:
            ...
            default_col = entry.get("column", current_columns[0] if current_columns else "")
            ensure_default(f"rename_col_{scope}_{file_key}_{i}", default_col)
            st.selectbox(
                "Column",
                current_columns or [""],
                key=f"rename_col_{scope}_{file_key}_{i}",
                on_change=_sync_rename_entry_from_widgets, args=(file_key, i, scope)
            )
        with cols[1]:
            ensure_default(f"rename_name_{scope}_{file_key}_{i}", entry.get("new_name", ""))
            st.text_input(
                "New name",
        ...
        with cols[2]:
            if st.button("🗑️", key=f"del_rename_{scope}_{file_key}_{i}", help="Delete rename entry"):
...
def add_schema_ui(df, file_key):
    ...
    for i in range(len(st.session_state.schema_entries[file_key])):
        ...
        with cols[0]:
            ensure_default(f"schema_entry_{file_key}_{i}", st.session_state.schema_entries[file_key][i].get("name", ""))
            schema_name = st.text_input("Schema column", key=f"schema_entry_{file_key}_{i}",
                                        placeholder="e.g. FinalColumnName")
        with cols[1]:
            ensure_default(f"default_value_{file_key}_{i}", st.session_state.schema_entries[file_key][i].get("default", ""))
            default_value = st.text_input("Default value (optional)",
                                          key=f"default_value_{file_key}_{i}",
                                          placeholder="e.g. N/A")
        with cols[2]:
            if st.button("🗑️", key=f"del_schema_{file_key}_{i}", help="Delete schema entry"):
                st.session_state.schema_entries[file_key].pop(i)
                st.rerun()
        if i < len(st.session_state.schema_entries[file_key]):
            st.session_state.schema_entries[file_key][i] = {"name": schema_name, "default": default_value}
        ...
        for col in selected_cols:
            if col in new_df.columns:
                if unified_name in new_df.columns:
                    new_df[unified_name] = new_df[unified_name].combine_first(new_df[col])
                else:
...
def blanks_per_file(df: pd.DataFrame, col: str) -> pd.DataFrame:
    ...
    if META_COL in df.columns:
        grp = df.loc[mask_blank].groupby(META_COL, dropna=False).size().reset_index(name="blank_count")
...
def apply_per_file_replacements(df: pd.DataFrame, col: str, per_file_map: dict) -> pd.DataFrame:
    ...
    for fname, replacement in per_file_map.items():
        ...
        if mask.any():
            if is_categorical_dtype(out[col]) and rep not in list(out[col].cat.categories):
                out[col] = out[col].cat.add_categories([rep])
            out.loc[mask, col] = rep
    return out

def recompute_grouped_from_melt_base(melt_base: pd.DataFrame, group_cols: list) -> pd.DataFrame:
    ...
    grouped_df = grp_base.groupby(dims, dropna=False, observed=True)['Values'].sum().reset_index()
    for c in dims:
...
def apply_blank_replacements_and_final_group(pre_group_base: pd.DataFrame,
                                             bcol: str,
                                             group_columns: list,
                                             per_file_map: dict) -> pd.DataFrame:
    """
    ...
    if per_file_map:
        map_series = out.loc[blank_mask, META_COL].map(lambda f: str(per_file_map.get(f, "")).strip())
        fill_mask = blank_mask & map_series.ne("")
        if fill_mask.any():
            if is_categorical_dtype(out[bcol]):
                new_vals = pd.Index(map_series[fill_mask].dropna().unique())
                to_add = [v for v in new_vals if v not in list(out[bcol].cat.categories)]
                if to_add:
...
def render_download_and_breakdown_melt(final_df: pd.DataFrame,
    ...
    breakdown_cols = [c for c in final_df.columns if c not in (META_COL, '_source_file_display', 'Values')]
    ...
    bcol = st.selectbox("Select the column to split the final output:",
    ...
    def get_blank_summary_and_gate(selected_col: str):
        ...
        grp = pre_group_base.loc[mask_blank].groupby(META_COL, dropna=False).size().reset_index(name="blank_count")
    ...
    if needs_save and not st.session_state.get(saved_flag_key, False):
        ...
        all_filled = len(inputs_map) > 0 and all(str(v).strip() for v in inputs_map.values())
        if st.button("💾 Save Replacements", key=f"save_repl_{state_key}_{bcol}", disabled=not all_filled):
            st.session_state[saved_flag_key] = True
            st.success("Saved replacements.")
        if not st.session_state.get(saved_flag_key, False):
            return

    per_file_map = st.session_state.breakdown_state.get(f"replacements_{state_key}_{bcol}", {})
    if per_file_map or needs_save:
        work_df = apply_blank_replacements_and_final_group(pre_group_base, bcol, group_columns, per_file_map)
    else:
        work_df = final_df
    ...
    with zipfile.ZipFile(zip_buffer, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        values = work_df_disp[bcol].astype(str).replace({"": "Blank"}).fillna("Blank").unique().tolist()
        for val in values:
            part = work_df_disp[work_df_disp[bcol].astype(str).replace({"": "Blank"}).fillna("Blank") == val]
...
if st.session_state.page == "process":
    ...
    if uploaded_files:
        ...
        st.subheader(f"Processing File {current_index + 1} of {total_files}: {file_key}")
        ...
        if not st.session_state.multi_mode:
            ...
            st.session_state.selected_metric_columns_single = st.session_state[metric_cols_key]
            ...
            if st.button("Apply Melt", key=f"apply_melt_single__{file_key}"):
                ...
                melted_df['Metrics'] = melted_df['MetricsOrig'].map(lambda x: cmap.get(x, x)).astype('category')
            ...
            if st.session_state.get(melt_applied_key) and not st.session_state.get(melted_key, pd.DataFrame()).empty:
                ...
                with st.form(f"group_form_{file_key}"):
                    group_columns = st.multiselect(
                        ...
                        [c for c in melted_df.columns if c not in ('Values', META_COL, '_source_file_display', 'MetricsOrig')],
                        key=group_cols_key
                    )
                ...
                if do_group:
                    if not group_columns:
                        ...
                        if grp_base.empty:
                            st.warning("All `Values` are NaN after melt. Check metric selection and numeric parsing.")
                ...
                if file_key in st.session_state.grouped_data and not st.session_state.grouped_data[file_key].empty:
        ...
        # ---------- MULTI-FILE FLOW ----------
            ...
            with nav_prev:
                if st.button("⬅️ Save & Previous", key=f"prev_file_{file_key}",
                             use_container_width=True, disabled=(current_index == 0)):
                    save_current_file(file_key)
                    st.session_state.file_index -= 1
                    st.rerun()
            with nav_next:
                if st.button("➡️ Save & Next", key=f"next_file_{file_key}", use_container_width=True):
                    save_current_file(file_key)
...
# ------------------------------------------------------------
    ...
    if not st.session_state.multi_mode:
        ...
        if not all_grouped:
            st.warning("No grouped data available. Please go back and process the file.")
            st.stop()
        combined_df = pd.concat(all_grouped, axis=0, ignore_index=True).fillna("")
        st.session_state.combined_df = combined_df
        selected_columns = st.multiselect("**Select columns to include in final download:**",
                                          [c for c in combined_df.columns if c not in (META_COL, '_source_file_display')])
        ...
        csv = decat_for_display(filtered_df).fillna("").to_csv(index=False).encode('utf-8')
        st.download_button("⬇️ Download Final Data", data=csv,
                           file_name="Processed_Combined_Data.csv", mime="text/csv")
    else:
        ...
        def update_mapping_multi(index):
            st.session_state.column_mappings_multi[index]["columns"] = st.session_state.get(f"merge_cols_multi_{index}", [])
            st.session_state.column_mappings_multi[index]["unified_name"] = st.session_state.get(f"unified_name_multi_{index}", "")
        ...
        for i, mapping in enumerate(st.session_state.column_mappings_multi):
            ...
            with cols[0]:
                st.multiselect(
                    ...
                    on_change=update_mapping_multi, args=(i,)
                )
            with cols[1]:
                st.text_input(
                    "Unified column name",
            ...
            with cols[2]:
                if st.button("🗑️", key=f"del_mapping_multi_{i}", help="Delete column mapping"):
                    st.session_state.column_mappings_multi.pop(i)
                    st.rerun()

        if st.button("➕ Add Column Mapping", key="add_column_mapping_multi"):
            st.session_state.column_mappings_multi.append({"columns": [], "unified_name": ""})
            st.rerun()

        harmonized_df = apply_column_mappings(combined_df, st.session_state.column_mappings_multi)
        ...
        metric_columns = st.multiselect("Select Metric columns to melt",
                                        numeric_candidates,
        ...
        if metric_columns:
            ...
            for col in metric_columns:
                clean_names.append(
                    st.text_input(f"Clean name for '{col}'",
                                key=f"clean_combined_{col}",
                                placeholder=f"e.g. {col}") or col
                )
            ...
            if st.button("Apply Melt", key="apply_melt_multi"):
                ...
                melted_df['Metrics'] = melted_df['Metrics'].map(metric_name_map).astype('category')
        ...
        if not st.session_state.final_melted.empty:
            ...
            st.write("🧮 **Final Output**")
            group_columns = st.multiselect("Select columns to include in final output",
                                           [c for c in melted_df.columns if c not in ('Values', META_COL, '_source_file_display')],
            ...
            if st.button("🚀 Group Data", key="group_data_combined"):
                if not group_columns:
                    ...
                    grp_base = melted_df.dropna(subset=['Values'])
                    if grp_base.empty:
                        st.warning("All `Values` are NaN after melt. Check metric selection/harmonization/number parsing.")
                        st.session_state.final_grouped = pd.DataFrame()
                    else:
                        dims = [c for c in group_columns if c in grp_base.columns]
                        for c in dims:
                            if grp_base[c].dtype == 'O':
                                grp_base[c] = grp_base[c].astype('category')
                        final_grouped = grp_base.groupby(dims, dropna=False, observed=True)['Values'].sum().reset_index()
                        for c in dims:

...
def run(params: dict):
    ...
    date_grain = params.get("date_grain", "weekly").strip().lower()  # allowed: daily|weekly|monthly
    QC_variables = [v.strip() for v in params.get("QC_variables", []) if v.strip()]
    columns_breakdown = [v.strip() for v in params.get("columns_breakdown", []) if v.strip()]
    ...
    priority_map = {"impressions": 1,
                    "sent": 2,"sents": 2, "send": 2, "sends": 2,
                    "delivered": 3,
                    "opens": 4, "open": 4,
                    "clicks": 5,
                    "cost": 6}
    ...
    if not has_long and raw_metrics:
        ...
        Metrics = sorted([col for col in df.columns if col.strip().lower() in wanted],
    ...
    if QC_variables:
        ...
        QC_combinations = list(itertools.product(*[QC_dict[f"QC{i}"] for i in range(len(QC_variables))]))
    else:
        QC_combinations = [()]

    out_path = Path(params.get("output_path") or csv_path.with_name(
        f"EDA_general_{pd.Timestamp.now():%H%M%S}.xlsx"
    ))
    ...
    with pd.ExcelWriter(out_path, engine="xlsxwriter") as writer:
        ...
        number_fmt = workbook.add_format({'num_format': '_(* #,##0_);_(* (#,##0);_(* "-"??_);_(@_)', 'border': 1})
        currency_fmt = workbook.add_format({
            'num_format': '_($* #,##0_);_($* (#,##0);_($* "-"??_);_(@_)',
            'border': 1
        })
        header_fmt   = workbook.add_format({
            'bold': True, 'bg_color': Tab_Color_Palette, 'font_color': 'white',
        ...
        for idx, combo in enumerate(QC_combinations, 1):
            ...
            valid_columns_breakdown = [
                col for col in columns_breakdown
                if col in df_f.columns and df_f[col].dropna().astype(str).str.strip().ne('').any()
            ]

            group_cols = [date_var] + QC_variables + valid_columns_breakdown
            if has_long:
                df_week_long = (
                    df_f.groupby(group_cols + [metric_var], as_index=False)[value_var].sum()
                )
                ...
                long = df_week.melt(
                    id_vars=id_vars, value_vars=Metrics,
            ...
            long['combined'] = (
                long[valid_columns_breakdown].agg(' '.join, axis=1) + ' ' + long['metric']
                if valid_columns_breakdown else long['metric']
            )
            ...
            for metric in Metrics:
                metric_cols = [c for c in pivot_week.columns if c.rsplit(' ', 1)[-1].lower() == metric.lower()]
                if metric_cols:
            ...
            final_week_cols = (
                [date_var] +
                [c for c in pivot_week.columns if c not in total_cols.values() and c != date_var] +
            ...
            if not pivot_week.empty:
                pivot_week[date_var] = pd.to_datetime(pivot_week[date_var], errors="coerce")
                ...
                if date_grain == "daily":
                    ...
                    idx = pd.date_range(dmin.normalize(), dmax.normalize(), freq=f"W-{anchor}" )
                pivot_week = (
                    pivot_week
                    .set_index(date_var)
                    .reindex(idx)
            ...
            if has_long:
                df_q_long = (
                    df_f.groupby(q_group + [metric_var], as_index=False)[value_var].sum()
                )
                ...
                q_long = df_q.melt(
                    id_vars=q_id, value_vars=Metrics,
                    var_name='metric', value_name='value'
                )
            q_long['combined'] = (
                q_long[valid_columns_breakdown].agg(' '.join, axis=1) + ' ' + q_long['metric']
                if valid_columns_breakdown else q_long['metric']
            )
            ...
            for metric in Metrics:
                metric_cols = [c for c in pivot_q.columns if c.rsplit(' ', 1)[-1].lower() == metric.lower()]
                if metric_cols:
                    total_name = f"Total {metric.capitalize()}"
            ...
            for col in list(pivot_q.columns):
                if col.lower().startswith("total "):
                    base_metric = col.replace("Total ", "").strip()
                    breakdown_cols = [c for c in pivot_q.columns if base_metric in c and not c.lower().startswith("total")]
                    if len(breakdown_cols) <= 1:
            ...
            if len(non_date_cols) == 2:
                total_col = next((col for col in non_date_cols if col.lower().startswith('total')), None)
                if total_col:
            ...
            pivot_q.to_excel(writer, sheet_name=short_sheet, index=False, startcol=q_startcol)
            check_incomplete_quarter(pivot_week, pivot_q, date_var, date_grain, short_sheet, ws, q_startcol)
            ...
            def fmt_cols(df_p, sc=0, is_q=False):
                for c, col in enumerate(df_p.columns):
                    ci = sc + c
                    vals = df_p[col].apply(lambda x: f"{x:,}" if isinstance(x, (int,float)) else str(x))
            ...
            for m_idx, metric in enumerate(Metrics):
                mcols = [c for c in pivot_week.columns if (metric.lower() in c.lower()) and not c.strip().lower().startswith("total ")]
                if not mcols: continue

                if len(mcols) <= 2:
                    chart = workbook.add_chart({'type': 'line'})
                    chart.set_title({
                        'name': pretty_col(metric) if pretty_col(short_sheet) == pretty_col(metric) else f"{pretty_col(short_sheet)} {pretty_col(metric)}",
                    ...
                    chart.set_y_axis({#'name': pretty_col(metric), 
                        'number_format': '#,##0', 
                        'line': {'none': True},
                        'major_gridlines': {'visible': False},
                        'minor_gridlines': {'visible': False}})
                    ...
                    for k, col in enumerate(mcols):
                        ...
                        chart.add_series({
                            ...
                            'line':       {'color': Color_Palette[k % len(Color_Palette)], 'width': 2},
                        })
                    ...
                    for k, col in enumerate(mcols):
                        ...
                        chart.add_series({
                            ...
                            'categories': f"='{short_sheet}'!$A$2:$A${num_rows+1}",
                            'values': f"='{short_sheet}'!${df_columns[loc]}$2:${df_columns[loc]}${num_rows+1}",
                            'fill': {'color': Color_Palette[k % len(Color_Palette)]},
                            'line': {'color': Color_Palette[k % len(Color_Palette)], 'width': 2},
                            'border': {'none': True}
                        })
            ...
            if cost_var and cost_var.strip():  # Only proceed if cost_var is provided
                total_cols = [c for c in pivot_week.columns if c.lower().startswith("total ")]
                cost_col = None
                if total_cols:
                    ...
                    for idx, base in enumerate(base_names):
                        if base.lower() == cost_var.lower():
                            ...
                            break
                if not cost_col:
                    cost_candidates = [c for c in pivot_week.columns if cost_var.lower() in c.lower()]
                    if cost_candidates:
                ...
                for metric in Metrics:
                    if metric.lower() == cost_var.lower():
                            continue
                    if total_cols:
                        primary_col = f"Total {metric.capitalize()}" if f"Total {metric.capitalize()}" in pivot_week.columns else None
                    else:
                    ...
                    chart = workbook.add_chart({'type': 'line'})
                    chart.set_title({'name': f"{pretty_col(primary_col)} vs {pretty_col(cost_col)}",'name_font': {'color': '#595959'} })
                    ...
                    chart.set_y_axis({#'name': pretty_col(primary_col),
                                    'number_format': '#,##0', 
                                    'line': {'none': True},
                                    'major_gridlines': {'visible': False},
                                    'minor_gridlines': {'visible': False}
                    ...
                    chart.set_y2_axis({#'name': pretty_col(primary_col),
                                    'number_format': '#,##0', 
                                    'line': {'none': True},
                                    'major_gridlines': {'visible': False},
                                    'minor_gridlines': {'visible': False}
        ...
        if has_long:
            df_grouped_long = (
                df_summary.groupby(group_cols_summary + [metric_var], as_index=False)[value_var].sum()
            )
            df_grouped = (
                df_grouped_long
                .pivot_table(
            ...
            metric_cols = Metrics
            df_grouped = (
                df_summary.groupby(group_cols_summary, as_index=False)[metric_cols].sum()
            )
        ...
        cost_format = workbook.add_format({'num_format': '_($* #,##0.00_);_($* (#,##0.00);_($* "-"??_);_(@_)'})
        # int_format  = workbook.add_format({'num_format': '#,##0'})
        int_format = workbook.add_format({'num_format': '_(* #,##0_);_(* (#,##0);_(* "-"??_);_(@_)'})
        ...
        ws_summary.set_column(col_indices['Month'], col_indices['Month'], 12, bold_format)
        ...
        for c, col in enumerate(df_grouped.columns):
            ws_summary.write(0, c, "Date" if col == date_var else pretty_col(col), header_fmt)

...
def data_preperation():
    ...
    def channel_selection_ui(df, file_key):
        ...
        channel_like = next(
            (col for col in eligible_columns if str(col).strip().lower() == "channel"),
            None,
        )
        ...
        channel_available = st.radio(
            "Is there a 'Channel' column?",
            ("Yes", "No"),
            key=f"channel_radio_{file_key}",
        )
        ...
        if channel_available == "Yes":
            ...
            ensure_default(
                f"custom_channel_col_{file_key}", saved.get("custom_name", "")
            )
    ...
    def add_schema_ui(df, file_key):
        ...
        if st.button("➕ Add Schema Column", key=f"add_schema_{file_key}"):
            st.session_state.schema_entries[file_key].append(
                {"name": "", "default": ""}
            )
            st.rerun()
                ...
                if total > 0 and invalid_ratio <= threshold:
                    ...
                    if s.dtype == "O":
                        df[col] = pd.to_numeric(
                            _preclean_numeric_strings(s), errors="coerce"
                        )
                    else:
    ...
    def cached_numeric_candidates(
        counts_sig: tuple, exclude_prefixes: tuple, threshold: float = 0.99
    ) -> list:
        out = []
    ...
    def build_pre_group_base(
        melt_base: pd.DataFrame, group_columns: list
    ) -> pd.DataFrame:
        """
        ...
        pre = (
            base.groupby(gcols, dropna=False, observed=True)["Values"]
            .sum()
            .reset_index()
        )
        for c in gcols:
    ...
    def apply_blank_replacements_and_final_group(
        ...
        for c in dims:
            if out[c].dtype == "O":
                out[c] = out[c].astype("category")
        final = (
            out.groupby(dims, dropna=False, observed=True)["Values"].sum().reset_index()
        )
        for c in dims:
    ...
    def render_download_and_breakdown_melt(
        ...
        if st.session_state.get("page") == "combine" and st.session_state.get(
            "multi_mode", False
        ):
            ...
            if not st.session_state.multi_mode:
                ...
                if st.button("Apply Melt", key=f"apply_melt_single__{file_key}"):
                    ...
                    melted_df = melt_base.melt(
                        id_vars=[
                            c for c in melt_base.columns if c not in metric_columns
                        ],
                    ...
                    if do_group:
                        if not group_columns:
                            ...
                            if grp_base.empty:
                                ...
                                dims = [
                                    c for c in group_columns if c in grp_base.columns
                                ]
                                for c in dims:
    ...
    # ------------------------------------------------------------
        ...
        if not st.session_state.multi_mode:
            ...
            filtered_df = (
                combined_df[selected_columns] if selected_columns else combined_df
            )
            ...
            if not st.session_state.preprocessed_data:
                st.warning(
                    "No preprocessed files found. Please go back and process files."
                )
                st.stop()
            ...
            cands = cached_numeric_candidates(
                counts_sig, tuple(EXCLUDE_MELT_PREFIXES), threshold=0.99
            )
            ...
            numeric_candidates = st.session_state.get(
                "cached_numeric_candidates_multi", []
            )
            numeric_candidates = filter_melt_candidates(numeric_candidates)
